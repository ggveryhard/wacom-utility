...
WAYLAND_INDEX_MAP_BY_MODEL = {
    "PTZ-630": {
        ...
        "striprup": 6,
        "striprdn": 7,
        # Kept for completeness from legacy docs/diagram.
        "Button9": 8,
        "Button10": 9,
        "Button11": 10,
        "Button12": 11,
...
def load_wacom_ascii_diagram() -> str:
    ...
    return (
        ...
        "| 0-Button3  1-Button1  2-Button2  3-Button8  4-StripLeftUp  5-StripLeftDown |\n"
        "| 6-StripRightUp  7-StripRightDown  8-Button9  9-Button10  A-Button11  B-Button12 |\n"
    )
...
class MainWindow:
    def __init__(self, app: Gtk.Application):
        ...
        self.pad_preview_box: Gtk.Box = self.builder.get_object("pad-preview-box")
        self.padbuttonlist: Gtk.ListBox = self.builder.get_object("padbuttonlist")
        self.pad_widget = Pad()
        self.pad_preview_box.append(self.pad_widget)
    ...
    def _configure_stack_pages(self) -> None:
        pages = [
            ...
            ("PadContainer", "pad", "Pad"),
            ("PressureContainer", "pressure", "Pressure"),
            ("MappingContainer", "mapping", "Mapping"),
    ...
    def refresh_pad_page(self) -> None:
        self.clear_listbox(self.padbuttonlist)
        self.pad_widget.set_parameters(self.current_tablet)
        self.pad_widget.queue_draw()

        if not self.current_tablet or not getattr(self.current_tablet, "Buttons", None):
            row = Gtk.ListBoxRow()
            label = Gtk.Label(label="No pad button description available for this model.")
            label.set_xalign(0.0)
            row.set_child(label)
            self.padbuttonlist.append(row)
            return

        for b in self.current_tablet.Buttons:
            action_name = "Unavailable"
            if self.current_device and self.backend.use_xsetwacom:
                _, action_name = self.backend.getTypeAndName(self.current_device.identifier, b.Callsign)
            elif self.backend.session_type == "wayland":
                _typ, action_name = self.get_wayland_mapping_for_button(b)
            wl_idx = wayland_button_index(
                self.current_tablet.Model if self.current_tablet else None,
                b.Callsign,
                b.Number,
            )
            wl_tag = f" | WL idx {wl_idx}" if wl_idx is not None else ""

            row = Gtk.ListBoxRow()
            line = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
            left = Gtk.Label(label=f"{b.Number}. {b.Name} ({b.Callsign}{wl_tag})")
            left.set_xalign(0.0)
            right = Gtk.Label(label=action_name)
            right.set_xalign(1.0)
            right.set_hexpand(True)
            edit_btn = Gtk.Button(label="Edit")
            edit_btn.connect("clicked", self.on_edit_button_clicked, b)
            line.append(left)
            line.append(right)
            line.append(edit_btn)
            row.set_child(line)
            self.padbuttonlist.append(row)
    ...
    def refresh_mapping_controls(self) -> None:
        ...
        self._set_dropdown_items(
            ...
            [f"{b.Number}. {b.Name} ({b.Callsign})" for b in self.current_tablet.Buttons],
        )
    ...
    def on_select_device(self, _listbox: Gtk.ListBox, row: Gtk.ListBoxRow | None) -> None:
        ...
        if "pad" in self.current_device.name.lower():
            self.stack.set_visible_child_name("pad")
